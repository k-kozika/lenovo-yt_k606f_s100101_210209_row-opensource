...
static int dmic_switch_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	...
	ucontrol->value.integer.value[0] = priv->dmic_switch_status;
	dev_info(va_dev, "%s: dmic switch is %s\n", __func__,
				priv->dmic_switch_status ? "DSPG" : "AP");

	return 0;
}

static int dmic_switch_put(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	...
	if (value == 1) {
		gpio_set_value(priv->dmic_switch_gpio, 1);
		priv->dmic_switch_status = 1;
		dev_info(va_dev, "%s: dmic switch set to DSPG\n", __func__);
	} else {
		gpio_set_value(priv->dmic_switch_gpio, 0);
		priv->dmic_switch_status = 0;
		dev_info(va_dev, "%s: dmic switch set to AP\n", __func__);
	}
	return 0;
}
...
static const char * const dmic_switch_ctl_text[] = {
	"AP", "DSPG",
};
...
static const struct soc_enum dmic_switch_ctl_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(dmic_switch_ctl_text),
			    dmic_switch_ctl_text);
...
static const struct snd_kcontrol_new va_macro_snd_controls[] = {
	...
	SOC_ENUM_EXT("DMIC_SWITCH", dmic_switch_ctl_enum,
			dmic_switch_get, dmic_switch_put),
};
...
static int va_macro_probe(struct platform_device *pdev)
{
	...
	if (of_parse_phandle(pdev->dev.of_node, micb_supply_str, 0)) {
		...
		ret = regulator_set_voltage(va_priv->micb_supply,
				      ...
				      va_priv->micb_voltage);
		if (ret) {
			dev_err(&pdev->dev, "%s: Setting voltage failed, err = %d\n",
				__func__, ret);
			return ret;
		}
		ret = regulator_set_load(va_priv->micb_supply,
					 va_priv->micb_current);
		if (ret) {
			dev_err(&pdev->dev, "%s: Setting current failed, err = %d\n",
				__func__, ret);
			return ret;
		}
		ret = regulator_enable(va_priv->micb_supply);
		if (ret) {
			dev_err(&pdev->dev, "%s: regulator enable failed, err = %d\n",
				__func__, ret);
			return ret;
		}
	}
	...
	va_priv->dmic_switch_gpio = of_get_named_gpio(pdev->dev.of_node,
							"dmic_switch_gpio", 0);
	if (!gpio_is_valid(va_priv->dmic_switch_gpio))
		dev_err(&pdev->dev, "%s: %s property is not found %d\n", __func__,
					"dmic_switch_gpio", va_priv->dmic_switch_gpio);
	ret = gpio_request(va_priv->dmic_switch_gpio, "dmic switch gpio");
	if (ret < 0) {
		dev_err(&pdev->dev, "%s: request dmic switch gpio failed\n", __func__);
		return ret;
	}
	dev_info(&pdev->dev, "%s: request dmic switch gpio success, gpio is %d",
					__func__, va_priv->dmic_switch_gpio);
	gpio_direction_output(va_priv->dmic_switch_gpio, 0);
	gpio_set_value(va_priv->dmic_switch_gpio, 1);
	va_priv->dmic_switch_status = 1;
